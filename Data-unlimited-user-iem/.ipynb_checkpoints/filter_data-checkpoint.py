...
def filter_data(filePath):
    ...
    choosed_index_del_i=rate_size_dic_i.index[rate_size_dic_i<0]
...
def filter_user(ratings1,ratings2):
    ...
    choosed_index_del_u1=rate_size_dic_u1.index[rate_size_dic_u1<0]
    choosed_index_del_u2=rate_size_dic_u2.index[rate_size_dic_u2<0]
...
data_dic = {'miRNA-disease':'miRNA-disease-rating','miRNA-target':'miRNA-target-rating'}
...
pprint('after common_data-miRNA_target info : %d %d %d %.6f'%(u,i,r,r/(u*i)),f)
...
pprint('filter way: user>%d,item>%d'%(1,40),f)
